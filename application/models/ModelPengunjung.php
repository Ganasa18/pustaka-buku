...
class ModelPengunjung extends CI_Model
{
  function statistik_pengujung()
  {
    $query = $this->db->query("SELECT DATE_FORMAT(pengunjung_tanggal,'%d') AS tgl,COUNT(pengunjung_ip) AS jumlah 
                               FROM pengunjung WHERE MONTH(pengunjung_tanggal)=MONTH(CURDATE()) 
  ...
  {
    $query = $this->db->query("SELECT DATE_FORMAT(pengunjung_tanggal, '%M') AS bulan,COUNT(pengunjung_ip) AS jumlah 
  ...
  {
    $hsl = $this->db->query("SELECT COUNT(pengunjung_ip) AS jumlah 
                             FROM pengunjung WHERE 
                             MONTH(pengunjung_tanggal)=MONTH(CURDATE())");
    return $hsl;
  }
  ...
  function get_sum_visitor_last_month()
  {
    $hsl = $this->db->query("SELECT COUNT(pengunjung_ip) AS jumlah 
                             FROM pengunjung WHERE 
                             MONTH(pengunjung_tanggal)=MONTH(DATE_SUB(CURDATE(), INTERVAL 1 MONTH))");
    return $hsl;
  }

  function get_average_perday()
  {
    $hsl = $this->db->query("SELECT COUNT(pengunjung_ip)/COUNT(DISTINCT DAY(pengunjung_tanggal)) AS jumlah 
                             FROM pengunjung WHERE 
                             MONTH(pengunjung_tanggal)=MONTH(CURDATE())");
    return $hsl;
  }

  function simpan_user_agent($user_ip)
  {
    $hsl = $this->db->query("SELECT * FROM pengunjung WHERE 
                             pengunjung_ip = '$user_ip' AND DATE(pengunjung_tanggal)=CURDATE()");
    return $hsl;
  }
  ...
  function count_visitor()
  {
    ...
    $cek_ip = $this->db->query("SELECT * FROM pengunjung WHERE
                                pengunjung_ip='$user_ip' AND DATE(pengunjung_tanggal)=
                                CURDATE()");
    if ($cek_ip->num_rows() <= 0) {
      $hsl = $this->db->query("INSERT INTO pengunjung (pengunjung_ip,pengunjung_perangkat) 
                               VALUES('$user_ip','$agent')");
      return $hsl;
    }
  }
}
